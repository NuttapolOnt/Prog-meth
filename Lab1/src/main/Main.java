...
public class Main {
	...
	public static boolean addDepartment(String newName){
		...
		try {
			if (newName.isBlank()) {
				throw new exception.EmptyGuildNameException();
			}
			else if(myDatabase.isExists(newName)) {
				throw new exception.DuplicateGuildNameException();
			}
			else {
			...
			catch (exception.EmptyGuildNameException e) {
				System.out.println(e.toString());
				return false;
			}
			catch (exception.DuplicateGuildNameException e) {
				System.out.println(e.toString());
				return false;
			}
	...
	public static void removeDepartmentFromDatabase(int index) {
		...
		String removedDepartment = (myDatabase.getDepartment(index)).getName();
		ArrayList<GuildMember> removedDepartmentMembers = myDatabase.removeDepartment(index);
		for (int i = 0; i < removedDepartmentMembers.size(); i++ ) {
			getUnassignedDepartment().addMember(removedDepartmentMembers.get(i));
		}
		System.out.println("Department removed. " + removedDepartmentMembers.size() + " member(s) added to Unassigned Department.");	
	}
